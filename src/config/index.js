...
var depth = 0;
var exported = false;
var outputFileName = 'structure.txt';
var searchPath = path.resolve(process.argv[2] || '.');
var key = searchPath;//.replace(/\//g,'');
var resolveWindow = path.resolve(searchPath).replace(/\\/g, '/')
var startFolder = resolveWindow.split('/')[resolveWindow.split('/').length - 1]
...
var generateMarkdown = async function(){
  addFolderName(key, 1);
  ...
  //await writeFile(currentWorkingDirectory + '/' + outputFileName, markdownText)
};
